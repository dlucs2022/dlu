<template>
  <div class="all">
    ...
    <!-- 设置按钮弹出对话框 -->
    <el-dialog title="绑定按键" :visible.sync="dialogVisible_setting_bind" width="35%">
      <el-table
        :data="label_c"
        style="width: 100%"
        max-height="250"
        ...
        size="small"
      >
        <el-table-column prop="father" label="父标签" width="120">
          <template slot-scope="scope">
            <el-tag size="medium">{{ scope.row.father }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="子标签" width="120">
          <template slot-scope="scope">
            {{ scope.row.children }}
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="绑定按键" width="120" prop="keyValue">
          <template slot-scope="scope">
        ...
        <el-table-column fixed="right" label="释放" width="80">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
    ...
    <!-- 删除按钮弹出的对话框 -->
    <el-dialog title="删除标签" :visible.sync="dialogVisible_setting_del" width="22%">
      <el-tabs v-model="del_tab_activeName" type="card" @tab-click="handleClick">
        <el-tab-pane label="一级标题" name="first">
          ...
          >
            ...
            <el-table-column label="父标签" width="80" align="center">
              <template slot-scope="scope">
    ...
    <div class="image_classification">
      <div class="image_classification_left">
        ...
        <div class="carouse">
          ...
          <!-- 描述列表 -->
          <el-descriptions
            v-if="imgList[current - 1].file != 'empty'"
            class="margin-top"
            :column="3"
            size="small"
            border
          ...
          <div class="image_classification_left_bottom">
            ...
            <el-input
              ...
              @focus="keyDownReview()"
              @blur="keyDown()"
              @keyup.enter.native="gotoIndexByInput"
              size="mini"
              style="width: 15%"
              clearable
            ></el-input>
      ...
      <div class="image_classification_right">
        ...
        <el-card class="box-card card_right" body-style="padding:5px 10px 10px 10px ">
          ...
          <div class="text item">
            <div class="handle_no_empty">
              ...
              <el-button
                ...
                plain
                size="small"
                @click="next_not_empty"
                >下一张非空<i class="el-icon-arrow-right el-icon--right"></i
              ></el-button>
            </div>
            <hr />
            <div class="handle_empty">
              ...
              <el-button
                ...
                size="small"
                @click="next_empty"
                >下一张空<i class="el-icon-arrow-right el-icon--right"></i
              ></el-button>
            </div>
          </div>
          <!-- 两个标题选择卡片 -->
              ...
              <el-collapse-item name="1">
                <template slot="title">
                  ...
                  <div class="el-collapse-item-buttons_f">
                    <!-- 上传标签对照表 -->
                    <el-tooltip content="上传标签对照表" placement="top">
                      <el-button
                        type="text"
                        size="medium"
                        @click.stop.prevent="click_upload_csv"
                    ...
                    <!-- 上传云数据库 -->
                    <el-tooltip content="上传数据库" placement="top">
                      <el-button
                        type="text"
                        size="medium"
                        @click.stop.prevent="downLoad_Sec_Label()"
                        icon="el-icon-cloudy"
                        class="el-collapse-item-button"
                      >
                      </el-button
                    ></el-tooltip>
                    <!-- 下载标签对照表 -->
                    <el-tooltip content="下载标签对照表" placement="top">
                      <el-button
                        type="text"
                        size="medium"
                ...
                <el-scrollbar style="height: 8vh">
                  <el-radio
                    v-model="current_label.label_f_current"
                    size="small"
                    border
                    @change="radio1Change"
                    style="margin-left: 0px; margin-right: 0px"
                    v-for="(item, index) in label_f"
                    :key="index"
                    :label="item"
                    @click.native.prevent="handleRadioClick_f(item)"
                  >
                  </el-radio>
                </el-scrollbar>
              </el-collapse-item>
              <el-collapse-item name="2" style="position: relative">
                <template slot="title">
                  ...
                  <div class="el-collapse-item-buttons">
                    <!-- 绑定按键 -->
                    <el-tooltip content="绑定按键" placement="top">
                      <el-button
                        type="text"
                        size="medium"
                        @click.stop.prevent="setting_Sec_Label()"
                        @click="dialogVisible_setting_bind = true"
                    ...
                    <el-tooltip content="删除标签" placement="top">
                      <el-button
                        ...
                        size="medium"
                        @click.stop.prevent="setting_Sec_Label()"
                        @click="dialogVisible_setting_del = true"
                        icon="el-icon-delete"
                        class="el-collapse-item-button"
                      ></el-button
                    ></el-tooltip>
                ...
                <div>
                  <el-scrollbar style="height: 20vh">
                    <el-radio
                      v-model="current_label.label_c_current"
                      size="mini"
                      border
                      style="margin-left: 0px; margin-right: 0px"
                      v-for="(item, index) in label_c"
                      :key="index"
                      @change="radio_c_change"
                      :label="item.children"
                      :disabled="judge_disabled(item)"
                    >
                      {{ item.children }}
                      <span style="color: white; background-color: #409eff">{{
                        item.keyValue.toString() == "" || item.keyValue.toString() == null
                          ? ""
                          : "[" + item.keyValue + "]"
                      }}</span>
                    </el-radio>
          ...
          <div class="card_right_bottom">
              ...
              <el-collapse v-model="activeNames_individual_attributes">
                <el-collapse-item title="个体属性" name="1">
                  <div style="margin-bottom: 8px">
                    年龄：<el-radio-group
                      v-model="current_label.label_age_current"
                      size="small"
                    >
                      <el-radio-button
                        v-for="(item, index) in label_age"
                        :key="index"
                        :label="item"
                      ></el-radio-button>
                    </el-radio-group>
                  ...
                  <div style="margin-bottom: 8px">
                    性别：<el-radio-group
                      v-model="current_label.label_sex_current"
                      size="small"
                    >
                      <el-radio-button
                        v-for="(item, index) in label_sex"
                        :key="index"
                        :label="item"
                      ></el-radio-button>
                    </el-radio-group>
                  ...
                  <div style="margin-bottom: 8px">
                    数量：<el-input-number
                      v-model="current_label.current_num"
                      size="small"
            ...
            <el-button type="danger" style="width: 20%" @click="empty_current_labels"
              >清空</el-button
            >
            <el-button
              type="success"
