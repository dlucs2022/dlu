  ...
  <div class="file_container">
        ...
        <div v-for="(item, index) in folderList" :key="item.path">
          <div class="folderContainer">
            <div
              v-if="item.type == 0"
              class="folderWrapper"
              @dblclick="ondblclickFile(item.path, item.file_name)"
            ...
            >
              <div class="folder">
                <div class="front">
                  <div
                    class="bar"
                    v-show="!Math.round((item.progress / item.file_number) * 100) == 0"
                  >
                    <el-progress
                      type="circle"
                      :width="70"
                      :stroke-width="8"
                      :percentage="Math.round((item.progress / item.file_number) * 100)"
              ...
              <div class="progress">{{ item.progress }}/{{ item.file_number }}</div>
            </div>
            <div v-else class="folderWrapper" @dblclick="down(item.fileUrl)">
              <img
                src="@/assets/imgs/file/fileImg.png"
                style="width: 100px; height: 90px; margin-top: -13px"
                @contextmenu.prevent.stop="rightClickFolder(index, item, $event)"
              />
              <div class="folderName">
                <span>{{
    ...
    <div class="add-folder-9" :style="folderStyle" v-show="folderShow">
      <div class="add-folder-1">
        ...
        <div style="border: 2px solid rgba(18, 17, 42, 0.07)"></div>
        <div class="add-folder-2" @click="updateFloder">重命名</div>
        <div style="border: 2px solid rgba(18, 17, 42, 0.07)"></div>
        <div class="add-folder-2" @click="attribute">属 性</div>
        <div style="border: 2px solid rgba(18, 17, 42, 0.07)"></div>
...
export default {
  ...
  methods: {
    ...
    queryFolderByFileName() {
      ...
      setTimeout(() => {
        dao
          .queryFolderByFileName(this.user.invite_code, this.queryParams.folderName)
          .then((res) => {
            this.folderList = res.data.data;
          });
        this.cardLoading = false;
